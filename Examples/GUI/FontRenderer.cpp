...
RenderObject Font::buildRenderObject(std::string text) {
	...
	for (const char & c : text) {
		...
		for (const GLuint & index : glyph.indices) {
			RO.VAO.indicies.push_back(lastIndex + index);
		}
...
RenderObject Font::buildRenderObjectWithAttributes(std::string text, VAO_Flags flags, std::function<std::vector<GLfloat>(const Font::RenderInfo &)> attributeCallback) {
	...
	for (const char & c : text) {
		...
		for (size_t i = 0; i < glyph.vertices.size(); i += 5) {
			std::vector<GLfloat> vertexData = attributeCallback({ font, {glyph.vertices[i] + pen[0], glyph.vertices[i + 1] + pen[1], glyph.vertices[2] }, glyphID, line, widths[line], widthMax, c });

			if (vertexData.size() != (RO.VAO.stride - 2)) {
				printf("Vertex Data is not correct size!  Expected %d, got %d", (RO.VAO.stride - 2), vertexData.size());
				return RO;
			}
