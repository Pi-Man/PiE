...
const VertexAttributeType POSITION_F(Usage::POSITION, ShaderTypes::VEC3);
const VertexAttributeType NORMAL_F(Usage::NORMAL, ShaderTypes::VEC3);
const VertexAttributeType COLOR3_F(Usage::COLOR, ShaderTypes::VEC3);
const VertexAttributeType UV2_F(Usage::UV, ShaderTypes::VEC2);

const VertexAttributeType POSITION_D(Usage::POSITION, ShaderTypes::VEC3D);
const VertexAttributeType NORMAL_D(Usage::NORMAL, ShaderTypes::VEC3D);
const VertexAttributeType COLOR3_D(Usage::COLOR, ShaderTypes::VEC3D);
const VertexAttributeType UV2_D(Usage::UV, ShaderTypes::VEC2D);
...
std::vector<char> getNormal(Vertex v1, Vertex v2, Vertex v3) {
	...
	if (p >= 0 && n >= 0 && ShaderTypes::container_of(v1.format.types[p].type) == ShaderTypes::container_of(v1.format.types[n].type) && ShaderTypes::count_of(v1.format.types[p].type) == 3 && ShaderTypes::count_of(v1.format.types[p].type) == ShaderTypes::count_of(v1.format.types[n].type)) {
		size_t offset = v1.format.offsets[v1.format.position_index];
		return calcNormal[ShaderTypes::container_of(v1.format.types[v1.format.position_index].type)](v1.data + offset, v2.data + offset, v3.data + offset);
	}
...
std::vector<char> getUV(Vertex v1) {
	...
	if (p >= 0 && uv >= 0 && ShaderTypes::container_of(v1.format.types[p].type) == ShaderTypes::container_of(v1.format.types[uv].type) && ShaderTypes::count_of(v1.format.types[p].type) == 3 && ShaderTypes::count_of(v1.format.types[uv].type) == 2) {
		size_t offset = v1.format.offsets[v1.format.position_index];
		return calcUV[ShaderTypes::container_of(v1.format.types[v1.format.position_index].type)](v1.data + offset);
	}
	return std::vector<char>();
}
...
void insertVertex(const Vertex & v, Lazy<std::vector<char>> & norm, VertexArrayObject & VAO) {
...
void VertexArrayObject::addTriangle(Vertex v1, Vertex v2, Vertex v3) {
	if (v1.format == v2.format && v2.format == v3.format && v3.format == format) {
		...
		Lazy<std::vector<char>> norm{ [&v1, &v2, &v3]() { return getNormal(v1, v2, v3); } };
...
void VertexArrayObject::addQuad(Vertex v1, Vertex v2, Vertex v3, Vertex v4) {
	if (v1.format == v2.format && v2.format == v3.format && v3.format == format && v4.format == format) {
		...
		Lazy<std::vector<char>> norm1{ [&v1, &v2, &v4]() { return getNormal(v1, v2, v4); } };
		Lazy<std::vector<char>> norm2{ [&v2, &v3, &v4]() { return getNormal(v2, v3, v4); } };
		Lazy<std::vector<char>> norm3{ [&norm1, &norm2, this]() { return avgNormals[ShaderTypes::container_of(format.types[format.normal_index].type)]({ norm1(), norm2() }); } };
...
void VertexArrayObject::addCuboid(Vertex v1, Vertex v2, Vertex v3, Vertex v4, Vertex v5, Vertex v6, Vertex v7, Vertex v8) {
	if (v1.format == v2.format && v2.format == v3.format && v3.format == format && v4.format == format) {
		...
		Lazy<std::vector<char>> norm1{ [&v4, &v3, &v1]() { return getNormal(v4, v3, v1); } }; // bottom
		Lazy<std::vector<char>> norm2{ [&v3, &v2, &v1]() { return getNormal(v3, v2, v1); } };
		Lazy<std::vector<char>> norm3{ [&v5, &v6, &v8]() { return getNormal(v5, v6, v8); } }; // top
		Lazy<std::vector<char>> norm4{ [&v6, &v7, &v8]() { return getNormal(v6, v7, v8); } };
		Lazy<std::vector<char>> norm5{ [&v1, &v2, &v5]() { return getNormal(v1, v2, v5); } }; // front
		Lazy<std::vector<char>> norm6{ [&v2, &v6, &v5]() { return getNormal(v2, v6, v5); } };
		Lazy<std::vector<char>> norm7{ [&v2, &v3, &v6]() { return getNormal(v2, v3, v6); } }; // right
		Lazy<std::vector<char>> norm8{ [&v3, &v7, &v6]() { return getNormal(v3, v7, v6); } };
		Lazy<std::vector<char>> norm9{ [&v3, &v4, &v7]() { return getNormal(v3, v4, v7); } }; // back
		Lazy<std::vector<char>> norm10{ [&v4, &v8, &v7]() { return getNormal(v4, v8, v7); } };
		Lazy<std::vector<char>> norm11{ [&v4, &v1, &v8]() { return getNormal(v4, v1, v8); } }; // left
		Lazy<std::vector<char>> norm12{ [&v1, &v5, &v8]() { return getNormal(v1, v5, v8); } };

		GLenum n_type = ShaderTypes::container_of(format.types[format.normal_index].type);
		Lazy<std::vector<char>> norm13{ [&norm11, &norm12, &norm5, &norm1, &norm2, n_type]() { return avgNormals[n_type]({ norm11(), norm12(), norm5(), norm1(), norm2() }); } };
		Lazy<std::vector<char>> norm14{ [&norm5, &norm6, &norm7, &norm2, n_type]() { return avgNormals[n_type]({ norm5(), norm6(), norm7(), norm2() }); } };
		Lazy<std::vector<char>> norm15{ [&norm7, &norm8, &norm9, &norm1, &norm2, n_type]() { return avgNormals[n_type]({ norm7(), norm8(), norm9(), norm1(), norm2() }); } };
		Lazy<std::vector<char>> norm16{ [&norm9, &norm10, &norm11, &norm1, n_type]() { return avgNormals[n_type]({ norm9(), norm10(), norm11(), norm1() }); } };
		Lazy<std::vector<char>> norm17{ [&norm12, &norm5, &norm6, &norm3, n_type]() { return avgNormals[n_type]({ norm12(), norm5(), norm6(), norm3 }); } };
		Lazy<std::vector<char>> norm18{ [&norm6, &norm7, &norm8, &norm3, &norm4, n_type]() { return avgNormals[n_type]({ norm6(), norm7(), norm8(), norm3(), norm4() }); } };
		Lazy<std::vector<char>> norm19{ [&norm8, &norm9, &norm10, &norm4, n_type]() { return avgNormals[n_type]({ norm8(), norm9(), norm10(), norm4() }); } };
		Lazy<std::vector<char>> norm20{ [&norm10, &norm11, &norm12, &norm3, &norm4, n_type]() { return avgNormals[n_type]({ norm10(), norm11(), norm12(), norm3(), norm4() }); } };
		...
		indicies.push_back(index + 3); // bottom
		indicies.push_back(index + 2);
		indicies.push_back(index + 0);
		...
		indicies.push_back(index + 4); // top
		indicies.push_back(index + 5);
		indicies.push_back(index + 7);

		indicies.push_back(index + 5);
		indicies.push_back(index + 6);
		indicies.push_back(index + 7);


		indicies.push_back(index + 0); // front
		indicies.push_back(index + 1);
		indicies.push_back(index + 4);
		...
		indicies.push_back(index + 1); // right
		indicies.push_back(index + 2);
		indicies.push_back(index + 5);
		...
		indicies.push_back(index + 2); // back
		indicies.push_back(index + 3);
		indicies.push_back(index + 6);
		...
		indicies.push_back(index + 3); // left
		indicies.push_back(index + 0);
		indicies.push_back(index + 7);
...
void VertexArrayObject::addPolygon(std::vector<Vertex> vertices) {
	...
	if (ShaderTypes::container_size_of(format.types[p].type) != 3) {
	...
	GLenum type = ShaderTypes::container_of(format.types[p].type);
...
void VertexArrayObject::addMesh(const std::vector<Vertex> & vertices, const std::vector<GLuint> & indices) {
	std::vector<Lazy<std::vector<char>>> vertexNormals(vertices.size());
	std::unordered_map<size_t, std::vector<Lazy<std::vector<char>>*>> normals;
	std::vector<Lazy<std::vector<char>>> triangleNormals;

	triangleNormals.reserve(indices.size() / 3);

	for (int i = 0; i < indices.size(); i += 3) {
		Lazy<std::vector<char>> norm([&vertices, &indices, i]() {return getNormal(vertices[indices[i]], vertices[indices[i + 1]], vertices[indices[i + 2]]); });
	...
	for (int i = 0; i < vertices.size(); i++) {
		Lazy<std::vector<char>> norm([type, i, &normals]() {
			std::vector<std::vector<char>> norms(normals[i].size());
			for (Lazy<std::vector<char>> * norm : normals[i]) {
...
void VertexArrayObject::bindBuffers() {
	...
	if (!gen) {
		...
		for (size_t i = 0; i < format.types.size(); i++) {
			glVertexAttribPointer(i, format.types[i].count, ShaderTypes::container_of(format.types[i].type), GL_FALSE, format.stride, (void*) offset);
