...
namespace PiE {
	...
	Matrix4f AxisAlignedRect::createGUIMatrix() {
		return Matrix4f::Identity()
			.scale({ {(float)getWidth(), (float)getHeight(), 1.0f, 1.0f} })
			.translate({ {(float)getXmin(), (float)getYmin(), 0} })
			.scale({ {2.0f, 2.0f, 1.0f, 1.0f} })
			.translate({ {-1.0f, -1.0f, 0.0f} });
	}

	#define V(val) val.transform<double>(resolve)

	double AxisAlignedRect::getXmin() {
		auto resolve = [this](Value & v) -> Optional<double> { return v(ctx, AXIS::X); };
	...
	double AxisAlignedRect::getXmid() {
		auto resolve = [this](Value & v) -> Optional<double> { return v(ctx, AXIS::X); };
	...
	double AxisAlignedRect::getXmax() {
		auto resolve = [this](Value & v) -> Optional<double> { return v(ctx, AXIS::X); };
	...
	double AxisAlignedRect::getYmin() {
		auto resolve = [this](Value & v) -> Optional<double> { return v(ctx, AXIS::Y); };
	...
	double AxisAlignedRect::getYmid() {
		auto resolve = [this](Value & v) -> Optional<double> { return v(ctx, AXIS::Y); };
	...
	double AxisAlignedRect::getYmax() {
		auto resolve = [this](Value & v) -> Optional<double> { return v(ctx, AXIS::Y); };
	...
	double AxisAlignedRect::getWidth() {
		auto resolve = [this](Value & v) -> Optional<double> { return v(ctx, AXIS::X); };
	...
	double AxisAlignedRect::getHeight() {
		auto resolve = [this](Value & v) -> Optional<double> { return v(ctx, AXIS::Y); };
	...
	Vec2d AxisAlignedRect::getCenter() {
		return { {getXmid(), getYmid()} };
	}

	Vec2d AxisAlignedRect::getMin() {
		return { {getXmin(), getYmin()} };
	}

	Vec2d AxisAlignedRect::getMax() {
		return { {getXmax(), getYmax()} };
	}

	Vec2d AxisAlignedRect::getTopLeft() {
		return { {getXmin(), getYmax()} };
	}

	Vec2d AxisAlignedRect::getBottomRight() {
	...
	const AxisAlignedRect::Vector & AxisAlignedRect::Vector::operator=(const Vec<Value, 2> & other) const {
		(*this)[0].get() = other[0];
		(*this)[1].get() = other[1];
		return *this;
	}
