...
namespace PiE {
	...
	VertexAttributeType::VertexAttributeType(Usage usage, Shader_Type type) : usage(usage), type(type), size(container_size_of(type) * count_of(type)), count(count_of(type)) {
